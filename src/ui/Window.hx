	...
	**/
	...
	public function recenter() {
		if ( win != null ) {
			...
			win.x = Std.int( ( GameUtil.wScaled - size.width ) / 2 );
			win.y = Std.int( ( GameUtil.hScaled - size.height ) / 2 );
		}
	}

	function clampInScreen() {
		if ( win != null && windowComp != null ) {
			win.x = hxd.Math.clamp( win.x, 0, GameUtil.wScaled - windowComp.window.innerWidth );
			win.y = hxd.Math.clamp( win.y, 0, GameUtil.hScaled - windowComp.window.innerHeight );
		}
	}
